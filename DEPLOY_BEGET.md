...
- `REQUIRED_CHANNELS_LINKS=...` — как показать пользователю названия и **рабочие ссылки** в `/start` и `/check`. Формат: записи разделяются `@@@`, внутри записи `Название канала|||https://t.me/...`.  
  Важно: в файле `.env` символ `#` начинает комментарий. Разделитель `###` **без двойных кавычек вокруг всей строки** обрежет значение до первого `#`, и ссылки не распарсятся. Используйте `|||`, либо оберните значение в `"..."` если оставляете `###`.

Если `REQUIRED_CHANNELS_LINKS` не задан или не распарсился, бот подставит **`deploy/required-channel-links.txt`** (по одной строке: `Название|||https://t.me/...`). Иначе покажет id из `REQUIRED_CHANNELS`.
